...
module.exports = cron.schedule('0,30 * * * *', async () => {
    ...
    let getNoticeListSQL =
        ...
        'INNER JOIN (SELECT notice_id, user_id FROM notice WHERE is_enable = true AND notice_period >= ?)SQ3 ON SQ.notice_id = SQ3.notice_id ' +
        'INNER JOIN (SELECT user_id FROM user WHERE deleted_at IS NULL)SQ4 ON SQ3.user_id = SQ4.user_id ' +
        'INNER JOIN line_login LL ON SQ4.user_id = LL.user_id';
